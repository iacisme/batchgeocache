...
_Last updated: Session 3 (Steps 1–3 complete; Step 4 is the active task)_
...
| 4 | Migrate/generalize `src/` modules (remove Calgary + Drive assumptions) | in progress | |
...
**Step:** #4 — Migrate/generalize `src/` modules (remove Calgary + Drive assumptions)

**Scope note:** `helper_functions.py` is **excluded** from this migration — it's deprecated
legacy code, unused by the rest of `src/`, and its logic already lives in `geocoder.py`. Only
the following 10 modules are in scope: `__init__.py`, `address_normalizer.py`, `config.py`,
`geocoder.py`, `inspect_results.py`, `io_utils.py`, `logging_utils.py`, `models.py`,
`package_processor.py`, `result_combiner.py`.

**Purpose:** Copy the 10 in-scope modules into the new BatchGeoCache repo's package
structure, generalizing them to work for any city rather than just Calgary. Step 3's audit
confirmed these modules have no Drive coupling to remove, so this step is purely about
de-Calgary-fying them.

**Subtasks:**
- [ ] Copy the 10 in-scope modules into the BatchGeoCache repo's package directory
- [ ] Generalize `config.py`: replace the Calgary-flavored `user_agent` default
      (`"calgary_address_geolookup"`) with a neutral default, or make it a required/caller-
      supplied field
- [ ] Generalize `address_normalizer.py`: confirm the normalization logic itself isn't
      Calgary-specific (only its docstring currently references "Calgary-style" abbreviations)
      and update the docstring accordingly
- [ ] Generalize `inspect_results.py`: the `CALGARY_FSA_PREFIXES`,
      `CALGARY_LAT_RANGE`/`CALGARY_LON_RANGE` sanity-check bounds are hardcoded to Calgary —
      decide whether to parameterize them per-city or make the geographic sanity check
      optional/pluggable
- [ ] Sweep migrated files once more for any remaining Calgary-specific strings, comments, or
      defaults missed above
- [ ] Produce a short summary of what changed per module, to hand off as the starting point
      for Step 5's Drive-coupling re-review
