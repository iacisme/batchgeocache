...
_Last updated: Session 4 (Steps 1–5 complete; Step 6 is the active task)_
...
- Step 5 completed: re-audited the migrated files (not just the original prototype) and
  confirmed `io_utils.py`'s one `google.colab.files` reference (a docstring for
  `zip_latest_package_archive()`) is the sanctioned OAuth-free download path from Section 1,
  distinct from the `google.colab.drive` coupling being screened for — noted explicitly
...
| 5 | Review migrated files for any remaining Drive coupling | done | Re-audited all 10 migrated modules + `__init__.py` + new `boundaries.toml` — confirmed zero Drive coupling post-migration; consistent with Step 3's original findings. |
...
**Step:** #6 — Build new Colab notebook (git-clone install + mirrored workflow)

**Purpose:** Produce the new BatchGeoCache Colab notebook that installs the package via
`!git clone` (per Section 1's locked-in architecture — no Drive, no PyPI) and mirrors the
existing prototype's working notebook flow (upload → normalize/geocode in packages →
inspect/resume → combine → zip → download → optional reset), now running against the
generalized, migrated `src/` package confirmed Drive-free in Step 5.

**Subtasks:**
- [ ] Draft the install cell(s): `!git clone` the BatchGeoCache repo + editable/plain
      `pip install` into the Colab VM, no Drive mount anywhere
- [ ] Draft the config cell: instantiate `GeocoderConfig` with a `/content/...` project root
- [ ] Draft the data-in cell: `google.colab.files.upload()` for the client's source address file
- [ ] Draft the processing loop: split into packages, run `PackageProcessor`, save via
      `io_utils.save_package_outputs()`, log via `PipelineLogger`, persist resume state via
      `StateManager`
- [ ] Draft the inspection cell: call `inspect_results()` (with a `boundary_name` param,
      defaulting to `"calgary"` until more boundaries exist)
- [ ] Draft the combine/export cell: `ResultCombiner.combine_all()` +
      `export_combined()`, then `zip_combined_outputs()`
- [ ] Draft the download cell: `google.colab.files.download()` on the combined zip
- [ ] Draft the optional reset cell: `io_utils.reset_project_files()`, clearly marked
      destructive/opt-in
- [ ] Confirm no cell anywhere references `/content/drive/...` or `google.colab.drive`
